    ...
    return FutureBuilder<Register>(
        ...
        builder: (context, AsyncSnapshot<Register> snapshot) {
          return FutureBuilder<UserResponse>(
              future: UserRepository().getUsersByIdRole(0),
              builder: (context, AsyncSnapshot<UserResponse> admins) {
                if (!admins.hasData) {
                  return SizedBox();
                }
                return Scaffold(
                      ...
                      onTap: () async {
                        ...
                        if (!snapshot.hasData) {
                          if (await RegisterRepository().registerTopic(Register(
                              ...
                              browseTopic: 0))) {
                            bool checkadmin;
                            for (var admin in admins.data.users) {
                              checkadmin = await NotificationRepository()
                                  .createNotification(Notifications(
                                content:
                                    "${widget.user.name.split(" ").last} đã yêu cầu đề tài ${widget.topic.name}",
                                image: widget.topic.image,
                                idStudent: admin.keyFirebase,
                                timeCreated: DateTime.now().toString(),
                              ));
                            }
                            if(checkadmin) {
                              print("Update admin success");
                            }else print("Update admin faile");
                              ...
                              ..showSnackBar(
                                SnackBar(
                                  content: Row(
                                    ...
                                    children: <Widget>[
                                      Text(
                                          "Đã gửi yêu cầu, chờ admin duyệt đề tài"),
                                      Icon(
                                        Icons.error,
                            ...
                            children: [
                              ...
                              Column(
                                children: [
                                  ...
                                  Text(widget.topic.acceptanceTime
                                      .substring(0, 10)),
                                ],
                              ),
                              Container(
